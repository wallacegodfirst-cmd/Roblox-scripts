...
local VOID_POS = Vector3.new(0, -15000, 0)
...
-- ── AUTO KILL ZOMBIES (workspace.Zombies.Zombie) ─────────────
local function doAutoKillZombies()
    ...
    for _, zombie in ipairs(zombieFolder:GetChildren()) do
        ...
        if zHum and zHRP and zHum.Health > 0 then
            ...
            pcall(function() zHRP.CFrame = CFrame.new(VOID_POS) end)
        end
    end
end
...
local function doAutoItemCycle()
    ...
    for _, item in ipairs(items) do
        ...
        task.wait(0.35)
        -- Attempt every pickup method
        if item:IsA("Tool") then
            pcall(function() item.Parent = LP.Backpack end)
        end
        local pp = item:FindFirstChildOfClass("ProximityPrompt", true)
        if pp then
            pcall(function() pp.Enabled=true; pp.HoldDuration=0; pp.MaxActivationDistance=50 end)
            pcall(fireprompt, pp)
        end
    ...
    task.wait(1.5)   -- cooldown to prevent runaway speed
...
local function findCarryTarget()
    ...
    -- Fall back to nearest living player
...
local function doAutoCarry()
    ...
    local hrp = getHRP(); if not hrp then return end
    pcall(function() hrp.CFrame = CFrame.new(root.Position + Vector3.new(0, 2, 3)) end)
    task.wait(0.05)
    local pp = findCarryPrompt(target)
    if pp then
        pcall(function() pp.Enabled=true; pp.MaxActivationDistance=20; pp.HoldDuration=0 end)
        pcall(fireprompt, pp)
    end
end
...
local function doCarryKill()
    ...
    local hrp = getHRP(); if not hrp then carryKillRunning = false; return end
    ...
    pcall(function() getHRP().CFrame = CFrame.new(VOID_POS) end)
    task.wait(1.5)
    if savedCF then
        pcall(function()
            local h = getHRP(); if h then h.CFrame = savedCF end
        end)
    end
    task.wait(0.5)
    ...
    if grabTimer >= 0.12 then
        ...
        if S.autoAttack then
            ...
            if tHRP then
                local hrp = getHRP()
                if hrp then
                    pcall(function() hrp.CFrame = CFrame.new(tHRP.Position + Vector3.new(0,2,3)) end)
                end
...
-- Auto Item runs in its own task (rate-limited internally)
task.spawn(function()
    while true do
        task.wait(0.1)
        if S.autoItem then
            task.spawn(doAutoItemCycle)
        end
    end
end)
...
TabAuto:CreateToggle({
    Name="Carry Kill (Carry → Void)", CurrentValue=false, Flag="carryKill",
...
TabProj:CreateButton({
    Name="Fire Alpha Now",
    Description="Instantly fires Project Alpha evacuate",
    Callback=function() fireProjectEvacuate("Project Alpha") end,
})

TabProj:CreateButton({
    Name="Fire Beta Now",
    Description="Instantly fires Project Beta evacuate",
    Callback=function() fireProjectEvacuate("Project Beta") end,
})

TabProj:CreateButton({
    Name="Fire Delta Now",
    Description="Instantly fires Project Delta evacuate",
...
TabInfo:CreateParagraph({
    ...
    Content = "F6 = Panic / kill script.\n\nAuto Item: TPs to each item, picks it up (E), TPs back. Rate-limited.\nCarry Kill: Carries target then voids them.\nAuto Grab: Items come to YOU — no teleporting.",
})
