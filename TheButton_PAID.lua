...
local Window = Rayfield:CreateWindow({
    ...
    Theme = {
        ...
        TabTextColor=Color3.fromRGB(210,210,230), SelectedTabTextColor=Color3.fromRGB(255,255,255),
        ElementBackground=Color3.fromRGB(16,20,36), ElementBackgroundHover=Color3.fromRGB(30,40,75),
        SecondaryElementBackground=Color3.fromRGB(14,18,32), ElementStroke=Color3.fromRGB(40,60,110),
        ...
        ToggleBackground=Color3.fromRGB(20,30,60), ToggleEnabled=Color3.fromRGB(220,30,50),
        ToggleDisabled=Color3.fromRGB(40,45,60), ToggleEnabledStroke=Color3.fromRGB(255,80,100),
...
local function addESP(obj,color,tag)
    ...
    if ex then
        if ex[2] and tag then local l=ex[2]:FindFirstChildOfClass("TextLabel"); if l then l.Text=tag end end
        return
    end
    ...
    if tag then
        ...
        bb.AlwaysOnTop=true; bb.Size=UDim2.new(0,160,0,26); bb.StudsOffset=Vector3.new(0,3.5,0); bb.Parent=obj
        local l=Instance.new("TextLabel",bb)
        l.Size=UDim2.fromScale(1,1); l.BackgroundTransparency=1; l.TextColor3=color
        l.TextStrokeTransparency=0; l.Font=Enum.Font.GothamBold; l.TextScaled=true; l.Text=tag
    end
    espStore[obj]={h,bb}
end
...
local S = {
    ...
    alwaysButton=false, autoGrab=false, grabRadius=40, autoFarm=false,
    autoAlpha=false, autoBeta=false, autoDelta=false,
    autoCarry=false, carryTarget=nil, savedCF=nil,
}
...
local function updateMineESP()
    ...
    for _,o in ipairs(mf:GetChildren()) do
        if o.Name:lower():find("mine") and (o:IsA("Model") or o:IsA("BasePart")) then
...
local function updateGhostESP()
    local g=WS:FindFirstChild("Ghosts"); if not g then return end
    for _,ghost in ipairs(g:GetChildren()) do
        if S.ghostESP then
            addESP(ghost, Color3.fromRGB(0,220,255), "GHOST: "..ghost.Name)
            for _,d in ipairs(ghost:GetDescendants()) do
                if d:IsA("BasePart") or d:IsA("MeshPart") then d.LocalTransparencyModifier=0.35 end
            end
        else removeESP(ghost) end
    end
end
...
local function doInfStam()
    ...
    for _,d in ipairs(bar:GetDescendants()) do
        if d:IsA("NumberValue") or d:IsA("IntValue") then
            stamMax[d]=math.max(stamMax[d] or 0, d.Value); d.Value=stamMax[d]
        end
    end
    local hum=getHum()
    if hum then for _,a in ipairs({"Stamina","CurrentStamina","stamina"}) do
        if hum:GetAttribute(a)~=nil then hum:SetAttribute(a,100) end end end
end
...
local function applyInvis(on)
    ...
    for _,p in ipairs(c:GetDescendants()) do
        if (p:IsA("BasePart") or p:IsA("MeshPart")) and p.Name~="HumanoidRootPart" then
            p.LocalTransparencyModifier=on and 1 or 0 end
    end
end
...
local function resetHitbox()
    for hrp,size in pairs(hbOriginal) do if hrp and hrp.Parent then hrp.Size=size; hrp.Transparency=1 end end
    hbOriginal={}
end
...
local function doBypassLock()
    ...
    for _,dm in ipairs(doorsF:GetChildren()) do
        for _,d in ipairs(dm:GetDescendants()) do
            if d:IsA("ProximityPrompt") then pcall(function() d.Enabled=true end); pcall(fireprompt,d)
            elseif d:IsA("BasePart") and d.Name:lower():find("barrier") then pcall(function() d.CanCollide=false end) end
        end
    end
end
    ...
    if downed then
        for _,d in ipairs(downed:GetChildren()) do
            for _,pp in ipairs(d:GetDescendants()) do
                if pp:IsA("ProximityPrompt") then pcall(function() pp.Enabled=true end); pcall(fireprompt,pp) end
            end
        end
    end
    local hum=getHum()
...
-- ██ PAID COMBAT
-- ═══════════════════════════════════════════════════════════
local function voidEnemy(hrp)
    if not hrp then return end
    pcall(function() hrp.CFrame=CFrame.new(VOID_POS) end)
    local bv=Instance.new("BodyVelocity")
    bv.Velocity=Vector3.new(0,200,0); bv.MaxForce=Vector3.one*9e9; bv.Parent=hrp
...
local function findNearest()
    ...
    for _,plr in ipairs(Players:GetPlayers()) do
        ...
        if d<bd then best=plr; bd=d end
    end
    return best
end
...
-- ── AUTO KILL NPCs (Chefs / Zombies in workspace) ───────────
...
local function doAutoKillNPC()
    ...
    for _,fn in ipairs(NPC_FOLDERS) do
        ...
        for _,e in ipairs(f:GetChildren()) do
            ...
            if eh and er and eh.Health>0 and (hrp.Position-er.Position).Magnitude<80 then voidEnemy(er) end
        end
    end
end

-- ═══════════════════════════════════════════════════════════
-- ██ ALWAYS GET BUTTON  (works from anywhere, always wins)
...
local function tryGetButton()
    ...
    if pp then pcall(function() pp.Enabled=true end); pcall(fireprompt,pp) end
    local cd=btn:FindFirstChildOfClass("ClickDetector")
    if cd then pcall(fireclick,cd) end
end

-- ═══════════════════════════════════════════════════════════
-- ██ AUTO GRAB ITEMS  (loose workspace items)
-- ═══════════════════════════════════════════════════════════
local function doAutoGrab()
    local hrp=getHRP(); if not hrp then return end
    for _,child in ipairs(WS:GetChildren()) do
        if not isItemModel(child) then continue end
        ...
        if pp then pcall(function() pp.Enabled=true end); pcall(fireprompt,pp)
        else
            local cd=child:FindFirstChildOfClass("ClickDetector",true)
            if cd then pcall(fireclick,cd)
            else pcall(function() p.CFrame=hrp.CFrame end) end  -- bring to me to trigger touch
        end
    end
end
...
local function doAutoFarm()
    local t={}
    for _,p in ipairs(Players:GetPlayers()) do if p~=LP and p.Character then t[#t+1]=p end end
    if #t==0 then return end
    farmIdx=(farmIdx%#t)+1
    local target=t[farmIdx]; local hrp=getHRP(); local tHRP=target.Character:FindFirstChild("HumanoidRootPart")
...
-- ═══════════════════════════════════════════════════════════
-- ██ AUTO PROJECT  (kill every NPC in the project folder)
-- workspace.Project Alpha/Beta/Delta → Gunner, Medic, Riot, Striker, Warden...
-- ═══════════════════════════════════════════════════════════
local function killProject(folderName)
...
-- ═══════════════════════════════════════════════════════════
-- ██ CARRY  ([Q] CARRY prompt) + CARRY KILL (TP to void)
-- ═══════════════════════════════════════════════════════════
...
local function nearestCarryable()
    -- prefer downed characters, fall back to players
    local hrp=getHRP(); if not hrp then return nil end
    local best,bd=nil,math.huge
    local downed=WS:FindFirstChild("DownedCharacters")
    if downed then
        for _,d in ipairs(downed:GetChildren()) do
            local r=d:FindFirstChild("HumanoidRootPart") or d:FindFirstChildWhichIsA("BasePart")
            if r then local dist=(hrp.Position-r.Position).Magnitude; if dist<bd then best=d; bd=dist end end
        end
    end
    if best then return best end
    local plr=findNearest(); return plr and plr.Character or nil
end
local function doAutoCarry()
    local target=nearestCarryable(); if not target then S.carryTarget=nil; return end
...
local function doCarryKill()
    ...
    hrp.CFrame=CFrame.new(VOID_POS)   -- carried player follows you to the void
    task.wait(1.2)
...
-- project / npc kill loop  (kills as they spawn)
task.spawn(function()
    while true do task.wait(0.25)
        pcall(function()
            if S.autoAlpha   then killProject("Project Alpha") end
            if S.autoBeta    then killProject("Project Beta")  end
            if S.autoDelta   then killProject("Project Delta") end
...
tVis:CreateToggle({Name="Item ESP  (loose workspace items)", CurrentValue=false, Callback=function(v) S.itemESP=v; if not v then clearAllESP() end end})
...
tVis:CreateToggle({Name="Ghost ESP", CurrentValue=false, Callback=function(v)
    S.ghostESP=v; if not v then local g=WS:FindFirstChild("Ghosts"); if g then for _,o in ipairs(g:GetChildren()) do removeESP(o) end end end
end})
tVis:CreateToggle({Name="No Dark", CurrentValue=false, Callback=function(v)
    ...
    if not v then
        ...
        for _,o in ipairs(Lighting:GetChildren()) do if o:IsA("ColorCorrectionEffect") or o:IsA("BloomEffect") then o.Enabled=true end end
    end
end})
...
tAut:CreateToggle({Name="Always Get Button  (always win)", CurrentValue=false, Callback=function(v) S.alwaysButton=v end})
...
tAut:CreateSlider({Name="Grab Radius", Range={10,150}, Increment=5, CurrentValue=40, Callback=function(v) S.grabRadius=v end})
...
tProj:CreateLabel("Auto-kills every NPC that spawns in the project")
...
tCar:CreateToggle({Name="Auto Carry  (nearest)", CurrentValue=false, Callback=function(v) S.autoCarry=v end})
tCar:CreateButton({Name="Carry Kill  (carry + TP to void)", Callback=function() task.spawn(function() pcall(doCarryKill) end) end})
tCar:CreateDivider()
...
tSet:CreateLabel("Wallace Chaos Hub  —  PAID")
