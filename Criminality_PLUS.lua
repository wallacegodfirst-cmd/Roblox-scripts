-- ============================================================
-- Wallace Chaos Hub  |  Criminality  —  PLUS Edition
...
LOOPS.heartbeat=RunService.Heartbeat:Connect(function()
    if S.speedHack then local hum=getHum(); if hum then pcall(function() hum.WalkSpeed=S.speed end) end
    elseif S.autoSprint then local hum=getHum(); if hum then pcall(function() if hum.WalkSpeed<24 then hum.WalkSpeed=24 end end) end end
...
local Window=Rayfield:CreateWindow({
    Name            = "Wallace Chaos Hub  |  Criminality  PLUS",
    Icon            = 0,
    ...
    ConfigurationSaving={Enabled=true, FolderName="WallaceChaos_Criminality", FileName="PLUS_Config"},
    KeySystem=false,
})
